...
```{r}
#this function returns estimate of $Y$ from truncating the svd, which
#is like correcting for the first k PCs
trunc.svd = function(Y.svd, k){
  Y.svd$u[,1:k,drop=F] %*% diag(Y.svd$d[1:k,drop=F],nrow=length(Y.svd$d[1:k,drop=F])) %*% t(Y.svd$v[,1:k,drop=F])
}
...
Data1 <- read.table("data/beforeCorrection_onlyExpGenes.txt.pedOrder.Transposed.allPopNorm", header=FALSE)
xx = matrix(as.numeric(as.matrix(Data1[-1,-(1:2)])),nrow=210)
```

check the first PC is, roughly, overall mean
```{r}
xx.svd=svd(xx)
xx.mean=apply(xx,1,mean)
plot(xx.mean,xx.svd$u[,1])
```

```{r}
xx.correct = correctY(xx, xx.svd$u[,1:20])
plot(xx.correct$betahat[1,],xx.correct$betahat.shrunk[1,])
abline(a=0,b=1,col=2)
plot(xx.correct$betahat[5,],xx.correct$betahat.shrunk[5,])
abline(a=0,b=1,col=2)
plot(xx.correct$betahat[15,],xx.correct$betahat.shrunk[15,])
abline(a=0,b=1,col=2)
plot(xx.correct$betahat[20,],xx.correct$betahat.shrunk[20,])
abline(a=0,b=1,col=2)
```
